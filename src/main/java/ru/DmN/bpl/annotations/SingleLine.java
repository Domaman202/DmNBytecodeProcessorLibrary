...
/**
 * Требует чтобы операции с классом производились в одну строку, запрещает запись класса в переменную (ИЗ-ЗА JAVAC ЖЕЛАТЕЛЬНО ВООБЩЕ В ОДНУ СТРОКУ ПИСАТЬ, А НЕ КАК В ПРИМЕРЕ)
 * <pre>
 *     {@code
 *     new CallBuilder()
 *      .arg("Вася")
